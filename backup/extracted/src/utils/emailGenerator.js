...
async function createAkbarMailbox(baseUrl, domain) {
  const base = (baseUrl || 'https://mail.akbarstore.biz.id').replace(/\/$/, '');
  const username = generateUsername();
  ...
  if (!res.data || !res.data.email || !res.data.mailbox_id) {
    throw new Error('AkbarMail /api/create returned invalid response: ' + JSON.stringify(res.data));
  }
  return {
    username,
...
if (require.main === module) {
  console.log("=== Email Generator Test ===");
  for (let i = 0; i < 5; i++) {
    const result = generateEmail('newssolor.com');
    const bday = generateRandomBirthday();
    console.log("  " + result.email + " | name: " + result.name + " | bday: " + bday.full);
  }
  console.log("\n=== Generator.email Test ===");
  for (let i = 0; i < 5; i++) {
    const result = generateEmail('generator.email');
    console.log("  " + result.email + " | name: " + result.name);
  }
}

module.exports = { generateUsername, generateEmail, generateRandomName, generateRandomBirthday, createAkbarMailbox };

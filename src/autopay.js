...
const { waitForWhatsAppOtpFromMuMu } = require('./utils/mumuOtp');
...
class ChatGPTAutopay {
  constructor(a) {
    ...
    this._threadId = a.threadId || 0;
    // COLOR_RULE.md — tag uses INFO color (Cyan)
    ...
    this.gopayCountryCode = String(a.gopayCountryCode || '62').replace(/\D/g, '') || '62';
    this.gopayPhone = String(a.gopayPhone || '').replace(/\D/g, '');
    if (this.gopayPhone.startsWith(this.gopayCountryCode) && this.gopayPhone.length > this.gopayCountryCode.length + 5) {
      this.gopayPhone = this.gopayPhone.slice(this.gopayCountryCode.length);
    }
    if (this.gopayPhone.startsWith('0')) {
      this.gopayPhone = this.gopayPhone.slice(1);
    }
    this.gopayDisplayPhone = '+' + this.gopayCountryCode + this.gopayPhone;
    ...
    this.adbPath = a.adbPath || process.env.MUMU_ADB_PATH || null;
    this.deviceSerial = a.deviceSerial || null;
    ...
    this._seenGopayOtps = new Set();
    this.log = logger.withContext(this._threadId, this.email);
  }
  ...
  async ['loginToChatGPT']() {
    ...
    try {
      ...
      this.log.info('Login: Authorize URL \u2713');
      const r = await i(q.url);
      this.log.info('Login: Auth session \u2713');
      let s = new Set();
      try {
        s = await k;
      } catch (E) {
        this.log.debug('Baseline OTP fetch failed: ' + E.message?.substring(0, 80));
      }
      const t = uuidv4();
      ...
      this.log.info('Login: Email submitted \u2713');
      ...
      let C = y;
      if (A) {
        this.log.info('Login: OTP route (no password)');
      } else {
      ...
      if (B.includes('/email-verification') || C === 'login_challenge') {
        ...
        if (M.size > 0) {
          this.log.info('Login: Baseline OTP excluded (' + M.size + ' codes)');
        }
        let N = false;
        ...
        for (let S = 0; S < O.length; S++) {
          ...
          this.log.info('Login: Waiting OTP... (' + T + 's)');
          await sleep(O[S]);
          let U;
          ...
          if (!V) continue;
          M.add(String(V));
          this.log.info('Login: Trying OTP ' + V + '...');
          ...
          if (X?.['continue_url']) {
            B = X.continue_url;
            N = true;
            this.log.info('Login: OTP \u2713');
            break;
          }
          const Y = X?.['error']?.['code'] || 'unknown';
          if (Y === 'wrong_email_otp_code') {
            this.log.warn('Login: OTP mismatch, retrying...');
            continue;
          }
      ...
      for (let a0 = 0; a0 < 3; a0++) {
        ...
        if (a0 === 0 && a2) {
          this.log.debug('Session keys: [' + Object.keys(a2).join(', ') + ']');
        }
      }
      if (!D) {
        this.log.debug('Cookies: ' + b.allCookieNames().length + ' captured');
        throw new Error('Access token gagal didapat');
      }
      this.accessToken = D;
      this.log.success('Login: Token \u2713');
  ...
  async ['_followOAuthChain'](a) {
    ...
    this.log.debug('OAuth chain done');
  ...
  async ['createCheckoutSession']() {
    try {
      ...
      const h = g.data;
      const _state = h?.['state'] || 'unknown';
      if (h?.['state'] !== 'eligible') {
        const i = new Error('[Checkout] Coupon: ' + _state + ' (HTTP ' + g.status + ')');
        i.noRetry = true;
        throw i;
      }
    } catch (j) {
      if (j.noRetry || j.hint) throw j;
      this.log.error('[Checkout] Coupon failed: ' + j.message);
      const noRetryErr = new Error('[Checkout] ' + j.message);
    ...
    if (d.status !== 200) {
      ...
      // checkout error status handled below
    ...
    // checkout success logged via coupon line
    return f;
  }
  ...
  async ['initStripeCheckout']() {
    ...
    this.log.debug('Stripe init \u2713' + (this.initChecksum ? ' (checksum: ' + this.initChecksum.substring(0, 10) + '...)' : ''));
    return b.data;
  }
  ...
  async ['createPaymentMethod'](a) {
    ...
    this.log.debug('Payment method \u2713 (' + this.paymentMethodId + ')');
    return c.data;
  }
  ...
  async ['confirmCheckout'](a) {
    ...
    this.log.success('Stripe confirmed \u2713');
    return d.data;
  }
  ...
  async ['followStripeRedirect'](a) {
    ...
    if (!b) {
      ...
      if (!b) {
        this.log.info('Waiting for redirect...');
        for (let t = 0; t < 6; t++) {
          await sleep(5000);
          ...
          if (u.status === 200) {
            ...
            this.log.info('Poll ' + (t + 1) + '/6: status=' + v?.['status']);
          }
        }
        if (!b) {
    ...
    if (!b) {
      ...
      this.log.warn('No redirect URL found \u2014 status=' + C.status + ' approval=' + C.approval_method + ' si=' + (C.setup_intent ? 'yes' : 'null') + ' pi=' + (C.payment_intent ? 'yes' : 'null'));
      this.log.debug('Dump: ' + D.substring(0, 1500));
      throw new Error('[GoPay] Stripe redirect URL not found after polling');
    }
    this.log.debug('Following Stripe redirect...');
    ...
    if (i.status === 302 || i.status === 301) {
      ...
      if (F) {
        ...
        this.log.debug('Midtrans SNAP \u2713 (' + this.midtransSnapId + ')');
  ...
  async ['getMidtransTransaction']() {
    ...
    this.log.debug('Midtrans transaction \u2713');
    return a.data;
  }

  async ['linkGoPay']() {
    const a = { 'type': 'gopay', 'country_code': this.gopayCountryCode, 'phone_number': this.gopayPhone };
  ...
  async ['gopayAuthorize'](a) {
    ...
    try {
      ...
      this.log.debug('GoPay validate-reference \u2713');
    } catch (f) {
      this.log.debug('Validate reference: ' + f.message);
    }
    ...
    this.log.debug('GoPay consent \u2713 (OTP sent to WhatsApp)');
    return c.data;
  }
  ...
  async ['handleGoPayOtpAndPin']() {
    let a;
    if (this._otpInputFn) {
      this.log.info('GoPay OTP + PIN: waiting OTP from external source...');
      a = await this._otpInputFn();
    } else if (this.adbPath && this.deviceSerial) {
      this.log.info('GoPay OTP: Reading OTP from WhatsApp (' + this.gopayDisplayPhone + ')...');
      a = await waitForWhatsAppOtpFromMuMu({
        adbPath: this.adbPath,
        deviceSerial: this.deviceSerial,
        
        timeoutMs: parseInt(process.env.MUMU_OTP_TIMEOUT_MS) || 120000,
        pollMs: 8000,
        initialDelayMs: 5000,
        
        onUpdate: (remaining) => {
          if (remaining > 0) {
            this.log.info('GoPay OTP: Waiting... (' + Math.ceil(remaining / 1000) + 's remaining)');
          }
        },
      });
    } else {
      // COLOR_RULE.md colors
      const m = { 'cyan': '\x1b[96m', 'yellow': '\x1b[93m', 'green': '\x1b[92m', 'reset': '\x1b[0m', 'bold': '\x1b[1m' };
      console.log('\n' + m.bold + m.yellow + '=======================================' + m.reset);
      console.log(m.bold + m.cyan + '  GoPay OTP Verification' + m.reset);
      console.log(m.yellow + '  OTP sent to WhatsApp: ' + this.gopayDisplayPhone + m.reset);
    ...
    this.log.debug('GoPay OTP validated \u2713');
    ...
    try {
      ...
      this.log.debug('GoPay PIN page \u2713');
    } catch (q) {
      this.log.debug('PIN page: ' + q.message);
    }
    ...
    this.log.debug('GoPay PIN verified \u2713');
    ...
    this.log.success('GoPay linking done \u2713');
    return l.data;
  }
    ...
    for (let a = 0; a < 12; a++) {
      ...
      try {
        ...
        if (c?.['gopay_account_id'] || c?.['payment_type'] === 'gopay' || c?.['status_code'] === '200') {
          this.log.debug('GoPay linking confirmed');
          return c;
        }
        this.log.debug('Waiting for linking... (' + (a + 1) + '/12)');
      } catch (d) {
        this.log.debug('Poll error: ' + d.message);
      }
    }
    this.log.debug('Linking poll timeout, proceeding...');
    return null;
  }
  ...
  async ['chargeGoPay']() {
    ...
    this.log.success('Charge initiated \u2713');
    return b.data;
  }
  ...
  async ['handleChargePin'](a) {
    ...
    if (!c) {
      this.log.debug('No payment reference found');
      this.log.debug('Waiting for charge processing...');
      await sleep(10000);
      return null;
    }
    ...
    try {
      ...
      this.log.debug('Payment validated \u2713');
    } catch (l) {
      this.log.debug('Payment validate: ' + l.message);
    }
    let d = null;
    try {
      ...
      d = m.data;
      this.log.debug('Payment confirmed \u2713');
    } catch (n) {
      this.log.debug('Payment confirm: ' + n.message);
    }
    ...
    if (f) {
      this.log.debug('Charge PIN challenge: ' + f.substring(0, 8) + '...');
      ...
      this.log.debug('Charge PIN verified \u2713');
      ...
      this.log.success('Payment processed \u2713');
      return t.data;
    }
    this.log.debug('Payment confirmed without PIN');
    return d;
  }
  ...
  async ['checkTransactionStatus']() {
    for (let c = 0; c < 18; c++) {
      try {
        ...
        this.log.debug('Status: ' + (e || 'unknown') + ' [' + (c + 1) + '/18]');
        if (e === 'settlement' || e === 'capture' || f === '200') {
          this.log.success('Settlement \u2713');
          return d.data;
        }
        ...
        if (g.code === 'ECONNABORTED' || g.message.includes('timeout')) {
          this.log.debug('Poll timeout, retrying... [' + (c + 1) + '/18]');
          await sleep(5000);
          continue;
        }
        throw g;
  ...
  async ['checkSubscriptionStatus']() {
    ...
    if (a.data?.['payment_status'] === 'paid' && a.data?.['status'] === 'complete') {
      this.log.success('Subscription: ' + a.data.plan_name + ' \u2713');
      return true;
    }
    return false;
  }
  ...
  async ['runAutopay']() {
    try {
      ...
      this.log.debug('Autopay error: ' + k);
      ...
      if (this.adbPath && this.deviceSerial) {
        try {
          ...
          const kill = (pkg) => new Promise((resolve) => {
            execFile(this.adbPath, ['-s', this.deviceSerial, 'shell', 'am', 'force-stop', pkg], { windowsHide: true, timeout: 5000 }, () => resolve());
          });
          await kill('com.gojek.gopay');
          await kill('com.whatsapp');
        } catch {}
      }
      await this.cleanup();
    }
